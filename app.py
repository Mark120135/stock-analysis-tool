...
def display_stock_final_tab(main_ticker, evaluator):
    ...
    with st.expander("Enter Quantitative Metrics"):
         ...
         with c1:
              ...
              user_inputs_final['quant_metrics']['current_ratio'] = st.number_input("Current R", value=float(sugg('current_ratio')), format="%.2f", help=f"Sugg: {sugg('current_ratio'):.2f}", key=f"final_current_ratio_{main_ticker}")
              user_inputs_final['quant_metrics']['quick_ratio'] = st.number_input("Quick R", 0.0, format="%.2f", key=f"final_quick_ratio_{main_ticker}")
              user_inputs_final['quant_metrics']['interest_coverage'] = st.number_input("Interest Cov", 0.0, format="%.2f", key=f"final_int_cov_{main_ticker}")
         ...
         with c2:
              ...
              user_inputs_final['quant_metrics']['revenue_growth_yoy'] = st.number_input("Rev Growth %", value=float(sugg('revenue_growth_yoy')*100), format="%.2f", help=f"Sugg: {sugg('revenue_growth_yoy'):.2%}", key=f"final_rev_g_{main_ticker}")
              ...
              user_inputs_final['industry_avgs']['pe'] = st.number_input("Ind Avg P/E", value=st.session_state.industry_avg_pe, format="%.2f", key=f"final_ind_pe_{main_ticker}")
    with st.expander("Enter Op Efficiency Metrics"):
         c1, c2 = st.columns(2)
         user_inputs_final['quant_metrics']['days_inventory'] = c1.number_input("Days Inv", 0.0, format="%.2f", key=f"final_days_inv_{main_ticker}")
         user_inputs_final['industry_avgs']['days_inventory'] = c2.number_input("Ind Avg Days Inv", 0.0, format="%.2f", key=f"final_ind_inv_{main_ticker}")
         user_inputs_final['quant_metrics']['days_sales'] = c1.number_input("Days Sales", 0.0, format="%.2f", key=f"final_days_sales_{main_ticker}")
         user_inputs_final['industry_avgs']['days_sales'] = c2.number_input("Ind Avg DSO", 0.0, format="%.2f", key=f"final_ind_dso_{main_ticker}")
